...
pub fn sys_socket(domain: u32, raw_ty: u32, proto: u32) -> AxResult<isize> {
    ...
    let ip_domain = if domain == AF_INET as u32 || domain == AF_INET6 as u32 {
        domain
    } else {
        AF_INET as u32
    };

    let inner = match (domain, ty) {
        ...
        (AF_UNIX, SOCK_STREAM) => SocketInner::Unix(Box::new(UnixSocket::new(StreamTransport::new(pid)))),
        (AF_UNIX, SOCK_DGRAM) => SocketInner::Unix(Box::new(UnixSocket::new(DgramTransport::new(pid)))),
        #[cfg(feature = "vsock")]
    ...
    let socket = Socket::new(inner, ip_domain);
...
pub fn sys_connect(fd: i32, addr: UserConstPtr<sockaddr>, addrlen: u32) -> AxResult<isize> {
    let addr =
        normalize_socket_addr_ex_for_ip_stack(SocketAddrEx::read_from_user(addr, addrlen)?, false)?;
...
pub fn sys_listen(fd: i32, backlog: i32) -> AxResult<isize> {
    ...
    Socket::from_fd(fd)?.listen(backlog as _)?;
...
pub fn sys_accept4(
    ...
    let ip_domain = listener.ip_domain();
    let socket = Socket::new(listener.accept()?, ip_domain);
...
pub fn sys_socketpair(
    ...
    let sock1 = Socket::new(SocketInner::Unix(Box::new(sock1)), AF_UNIX as u32);
    let sock2 = Socket::new(SocketInner::Unix(Box::new(sock2)), AF_UNIX as u32);
