...
fn task_maps(task: &AxTaskRef) -> String {
    let Some(aspace) = task.as_thread().proc_data.aspace.try_lock() else {
...
impl SimpleDirOps for ThreadDir {
    ...
    fn lookup_child(&self, name: &str) -> VfsResult<NodeOpsMux> {
        ...
        Ok(match name {
            ...
            .into(),
            "maps" => SimpleFile::new_regular(fs, move || Ok(task_maps(&task))).into(),
...
fn builder(fs: Arc<SimpleFs>) -> DirMaker {
    ...
    root.add(
        "syscall_stats",
        SimpleFile::new_regular(fs.clone(), || Ok(stats::format_stats_text())),
    );
    root.add(
        "syscall_stats_total",
        SimpleFile::new_regular(fs.clone(), || Ok(stats::format_stats_total_line())),
    );
    root.add(
        "syscall_stats_reset",
        SimpleFile::new_regular(
            ...
            RwFile::new(|req| match req {
                SimpleFileOperation::Read => Ok(Some(
                    b"write any bytes to reset syscall counters\n".to_vec(),
                )),
    ...
    root.add(
        ...
        SimpleFile::new_regular(
            ...
            RwFile::new(|req| match req {
                SimpleFileOperation::Read => {
                    Ok(Some(stats::format_trace_control().as_bytes().to_vec()))
                }
                SimpleFileOperation::Write(data) => {
                    if stats::write_trace_control(data) {
                        Ok(None)
                    } else {
    ...
    root.add(
        "syscall_trace_snapshot",
        SimpleFile::new_regular(
            fs.clone(),
            RwFile::new(|req| match req {
                SimpleFileOperation::Read => {
                    Ok(Some(stats::format_snapshot_control().as_bytes().to_vec()))
                }
                SimpleFileOperation::Write(data) => {
                    if stats::write_snapshot_control(data) {
                        Ok(None)
                    } else {
                        Err(VfsError::InvalidInput)
                    }
    ...
    root.add(
        ...
        SimpleFile::new_regular(
            ...
            RwFile::new(|req| match req {
                SimpleFileOperation::Read => {
                    Ok(Some(stats::format_deep_trace_control().as_bytes().to_vec()))
                }
                SimpleFileOperation::Write(data) => {
                    if stats::write_deep_trace_control(data) {
                        Ok(None)
                    } else {
                        Err(VfsError::InvalidInput)
                    }
    ...
    root.add(
        "syscall_trace_recent",
        SimpleFile::new_regular(fs.clone(), || Ok(stats::format_trace_recent_text())),
    );
    root.add(
        "deep_trace_recent",
        SimpleFile::new_regular(fs.clone(), || Ok(stats::format_deep_trace_recent_text())),
    );
    root.add(
        "syscall_inflight",
        SimpleFile::new_regular(fs.clone(), || Ok(stats::format_inflight_text())),
    );
    root.add(
        "task_snapshot",
        SimpleFile::new_regular(fs.clone(), || Ok(stats::format_task_snapshot_text())),
    );
    root.add(
        "task_block_snapshot",
    ...
    );
    {
        ...
        ax_task::register_timer_callback(|now| {
            IRQ_CNT.fetch_add(1, Ordering::Relaxed);
            stats::record_deep_timer_tick(now.as_secs());
        });
