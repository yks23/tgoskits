...
static int read_image_raw(const char* path, image_buffer_t* image)
{
    ...
    // 对于.data文件，我们需要根据文件大小推断图像尺寸
    // 假设是RGB格式（3通道），则 width * height * 3 = file_size
    // 在Yolov8的情况下，输入通常是640x640
    if (image->width == 0 && image->height == 0) {
        // 尝试推断尺寸，假设是正方形图像
        int pixels = file_size / 3;  // 3通道RGB
        int side = (int)sqrt(pixels);
        if (side * side * 3 == file_size) {
            image->width = side;
            image->height = side;
        } else {
            // 如果无法精确推断，使用默认的640x640（Yolov8模型输入尺寸）
            image->width = 640;
            image->height = 640;
            printf("Warning: Cannot infer image dimensions from file size, using default 640x640\n");
        }
    }

    // 确保格式已设置
    if (image->format == 0) {  // 0表示未设置格式
        image->format = IMAGE_FORMAT_RGB888;
    }
...
int read_image(const char* path, image_buffer_t* image)
{
    ...
    if (strcmp(_ext, ".data") == 0) {
        ...
        // 对于.data文件，需要设置图像格式为RGB888
        if (ret == 0 && image->format == 0) {  // 0表示未设置格式
            image->format = IMAGE_FORMAT_RGB888;
        }
        return ret;
#ifndef DISABLE_LIBJPEG
